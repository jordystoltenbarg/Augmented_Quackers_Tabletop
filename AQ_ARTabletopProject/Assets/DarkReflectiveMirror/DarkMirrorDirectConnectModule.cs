...
public class DarkMirrorDirectConnectModule : MonoBehaviour
{
    ...
    public bool ServerSend(int clientID, ArraySegment<byte> data, int channel)
    {
        //directConnectTransport.DRServerSend(clientID, channel, data);
        List<int> conncetionIds = new List<int>();
        conncetionIds.Add(clientID);
        return directConnectTransport.ServerSend(conncetionIds, channel, data);
    }

    public bool ClientSend(ArraySegment<byte> data, int channel)
    {
        //directConnectTransport.DRClientSend(channel, data);
